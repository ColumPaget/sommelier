...
static void PrintUsage()
{
    ...
    printf("  -s   <name>=<value>           used with 'install' to set a value permanently for an application\n");
    printf("  -set <name>=<value>           used with 'install' to set a value permanently for an application\n");
    ...
    printf("                                If used with action 'install' then the program is installed WITHOUT the ability to su.\n");
    printf("  -net                          Deny network access (using linux namespaces) even for platforms/apps that are configured to have it.\n");
    printf("  -nonet                        Deny network access (using linux namespaces) even for platforms/apps that are configured to have it.\n");
    printf("  +net                          Allow network access even for platforms/apps that are configured to not have it.\n");
    printf("  -pid                          Deny access to other processes (using linux namespaces).\n");
    printf("  -nopid                        Deny access to other processes (using linux namespaces).\n");
    printf("  +pid                          Allow network access even for platforms/apps that are configured to not have it.\n");
    printf("  -client                       Deny TCP and UNIX server sockets using linux seccomp (udp/icmp cannot be blocked).\n");
    printf("  -secure <security string>     Set libUseful 'ProcessApplyConfig' security level.\n");
    printf("  -security <security string>   Set libUseful 'ProcessApplyConfig' security level.\n");
    ...
    printf("\n");
    printf("Settings: These are set with the 'set' command, or with the 'install' command and '-s' or '-set' option\n");
    printf("ALL APP TYPES\n");
    printf("  allow-su=y/n            allow/deny application to raise priviledges to root user\n");
    printf("  allow-net=y/n           allow/deny application to access network (uses namespaces to deny)\n");
    printf("  allow-pid=y/n           allow/deny application to send signals or otherwise access other processes (uses namespaces to deny)\n");
    printf("  fullscreen=y/n          run in fullscreen mode (works in at least wine, msdos and DOOM)\n");
    printf("WINE ONLY\n");
    ...
    printf("DOOM ONLY\n");
    printf("  sound=y/n/sfx           sound on/off, or only effects (no music)\n");
    printf("  mouse=y/n               use mouse in-game, or not\n");
    printf("  grab=y/n                grab mouse, or not\n");
    printf("  vdesk=<resolution>      resolution of DOOM window\n");
    printf("\n");
...
static int ParseCommandLineOption(TAction *Act, CMDLINE *CmdLine)
{
    ...
    else if (strcmp(p_Opt, "-nosu")==0) Config->Flags |= CONF_DENY_SU;
    ...
    else if (strcmp(p_Opt, "+pids")==0) Config->Flags |= CONF_ALLOW_NET;
    else if (strcmp(p_Opt, "-pids")==0) Config->Flags |= CONF_DENY_NET;
    else if (strcmp(p_Opt, "-nopid")==0) Config->Flags |= CONF_DENY_NET;
    else if (strcmp(p_Opt, "-client")==0) Config->Flags |= CONF_DENY_SERVER;
